    ...
    internal class TextMapleStory
    {
        ...
        static void Main()
        {
            ...
            try
            {
                ...
                // 상점아이템 세팅 (불러온 후에 설정)
                shopItems.Add(new ShopItem(new Item("머쉬맘의 포자", StatType.체력, 50, "[튜토리얼 보스]머쉬맘의 포자이다. 머리에 쓰면 든든할 것 같다.", EquipmentType.모자)));
        ...
        static void ItemUpgrade()
        {
            ...
            // 강화할 아이템 선택
            Console.WriteLine("[강화 가능한 아이템 선택]:");

            for (int i = 0; i < player.Inventory.Count; i++)
            {
                var item = player.Inventory[i];
                int itemPrice = item.Power * 1000; // 가격 계산 (전투력 * 1000)

                // 강화 전 아이템 상태 출력
                string itemDetails = $"{i + 1}. {item.Name} | {item.Type} | 능력치: {item.Power} | 가격: {itemPrice} 메소";
                Console.WriteLine(itemDetails);
            }

            // 아이템 선택 후 강화 시도
            Console.WriteLine("강화할 아이템 번호를 입력해주세요:");
            string? input = Console.ReadLine();
            int choice;

            if (!int.TryParse(input, out choice) || choice < 1 || choice > player.Inventory.Count)
            {
                Console.WriteLine("잘못된 입력입니다.");
                return;
            }

            var selectedItem = player.Inventory[choice - 1];  // 선택한 아이템

            // 강화 확률 (단계별로 성공 확률, 실패 확률, 하락 확률을 다르게 설정)
            Random rand = new Random();
            int successChance = 80 - (choice - 1) * 2; // 강화 레벨에 따라 성공 확률 감소
            int failChance = 15 + (choice - 1) * 2;
            int downgradeChance = 5 + (choice - 1) * 1;
            int statBoost = rand.Next(1, 6); // 강화 수치 (1~5)
            int randomValue = rand.Next(1, 101); // 1부터 100까지의 난수 생성

            if (randomValue <= successChance)
            {
                // 강화 성공
                Console.WriteLine($"{selectedItem.Name} 강화에 성공했습니다!");

                // 능력치 증가 (강화된 능력치 출력)
                selectedItem.Power += statBoost; // 예시로 statBoost만큼 증가
                Console.WriteLine($"능력치:{selectedItem.Power} +{statBoost}");
                ShowInventory();
            }
            ...
            {
                // 강화 실패 후 능력치 하락
                Console.WriteLine($"{selectedItem.Name} 강화에 실패하여 능력치가 하락했습니다!");

                // 능력치 하락
                selectedItem.Power -= 1;
                Console.WriteLine($"강화 실패로 능력치가 1 감소했습니다. 현재 능력치: {selectedItem.Power}");
                ShowInventory();
            }
        }
